#!/usr/bin/env python3
"""
Re-run the Harvest steps (harvest_build → harvest_call → harvest_extract → verdict)
on `tmp/enriched.csv` in chunks of N rows, with a small delay between chunks, so
we never have more than N concurrent `generic_http_request` calls in flight at
Harvest's API. Reuses the existing `research` cells — no deeplineagent re-spend.

Why this exists:
    Harvest API throttles per-key concurrency. When Deepline fans out to all 28
    eligible rows in parallel from a single playbook run, Harvest returns
    HTTP 200 with body `{"error":"Too many queued requests (code_22)",
    "status":429}` on most/all of them. Chunking to N=5 (default) keeps us
    safely under the per-key cap.
...
How:
    For each chunk of `--rows START:END`, runs:
        deepline enrich --in-place
            ...
            --with-force harvest_build,harvest_call,harvest_extract,verdict
            --rows START:END

    The `--with-force` flag tells Deepline to recompute the named aliases even
    though they already have values (from the prior run that hit the rate limit).
    `--rows` restricts execution to that range; everything else stays put.
...
ALIASES = "harvest_build,harvest_call,harvest_extract,verdict"
...
def main() -> int:
    ...
    for i, (start, end) in enumerate(chunks):
        ...
        rc = run_chunk(args.enriched, args.playbook_compiled, start, end)
        if rc != 0:
